...
def deblur_module(pic, filename, dest_path, blur_width, confidence=10, tolerance=1, quality="normal", bits=8,
        ...
        for case in ["blind", "non-blind"]:
            ...
            for i, k in zip(reversed(images), reversed(kernels)):
                ...
                if True:  # case == "blind" or i == 1.:
                    ...
                    deblured_image = resize(deblured_image, shape, order=3, mode="edge", preserve_range=True).astype(
                        np.float32)
                    ...
                    if case == "blind":
                        deblured_image[temp_top - 1:temp_bottom + 1, temp_left - 1:temp_right + 1,
                        ...] = dc.richardson_lucy_MM(
                            temp_blurry_image[temp_top - 1:temp_bottom + 1, temp_left - 1:temp_right + 1, ...],
                            deblured_image[temp_top - pad - 1:temp_bottom + pad + 1,
                            temp_left - pad - 1:temp_right + pad + 1, ...],
                            psf_copy,
                            ...
                            3,
                            k, iterations, step, lambd, blind=True, p=p_temp, correlation=correlation, order=order,
                            norm=2, priority=0, refocus=refocus)
                        # Update the PSF
                        psf = psf_copy.copy()
                        ...
                        ...] = dc.richardson_lucy_MM(
                            ...
                            temp_right - temp_left + 2,
                            3,
                            k, iterations, step, lambd, blind=False, p=p_temp, order=order, norm=2, priority=priority,
                            refocus=refocus)
                    else:
                        # Pad for FFT
            ...
            if display and case == "blind":
                ...
                plt.show()
                plt.imshow((deblured_image[top:bottom, left:right, ...] * 255).astype(np.uint8),
                           interpolation="lanczos", filternorm=1, aspect="equal", vmin=0, vmax=255)
                plt.show()
    ...
    utils.save(deblured_image, filename[:-4], dest_path)
    # deblured_uint = deblured_image.astype(np.uint16)
    # print(deblured_uint)
    # print(deblured_uint.shape)
    # print(deblured_uint.shape[2])
    # deblured_jpg = Image.fromarray(deblured_uint, mode="RGB")
    # deblured_jpg.save(dest_path + "/" + filename + ".jpg")
